...
/** Layout and a11y only; let callers set backgrounds so hover matches sibling nav links after merge. */
const navigationMenuTriggerStyle = cva(
  "group/navigation-menu-trigger inline-flex h-auto min-h-0 w-max items-center justify-center rounded-xl px-4.5 py-2.5 text-sm font-medium outline-none transition-colors duration-200 ease-out focus:outline-none focus-visible:outline-none focus-visible:ring-0 disabled:pointer-events-none disabled:opacity-50"
)
...
function NavigationMenuTrigger({
  ...
  return (
    ...
    >
      ...
      <svg
        viewBox="0 0 28 28"
        className="relative top-px ml-1 size-[0.95rem] transition duration-300 group-data-popup-open/navigation-menu-trigger:rotate-180 group-data-open/navigation-menu-trigger:rotate-180"
        aria-hidden="true"
      >
        <path
          d="M4.5 9.5L14 18.5L23.5 9.5"
          fill="none"
          stroke="currentColor"
          strokeWidth="2.8"
          strokeLinecap="round"
...
function NavigationMenuLink({
  ...
  return (
    <NavigationMenuPrimitive.Link
      ...
      className={cn(
        "flex items-center gap-1.5 rounded-xl p-3 text-base transition-colors duration-200 ease-out outline-none hover:bg-muted focus:bg-muted focus-visible:ring-3 focus-visible:ring-ring/30 focus-visible:outline-1 in-data-[slot=navigation-menu-content]:rounded-xl data-[active=true]:bg-muted/50 data-[active=true]:hover:bg-muted data-[active=true]:focus:bg-muted [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    />
